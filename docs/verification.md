...
| 复审 gate 拆哨兵（stageHashes 区分「读失败」vs「已删」） | `stageHashes` catch 按 `isMaxBufferError` 拆分：超限读失败记 `"unreadable"`（强制 confirm），普通读不到记 `"deleted"`（文件已删）；`decide` 对 `"unreadable"` 天然 mismatch→confirm（fail-closed）；新增 2 用例（stageHashes 超限→unreadable + decide 读失败→confirm） | 🟢 | 2026-08-20 |
| stageHashes 64MB maxBuffer 上限（后置） | `review-gate.mjs` `git show :<path>` 仍用 `execFileSync` + 64MB maxBuffer；>64MB staged 文件读失败已 fail-closed 记 `"unreadable"` 强制 confirm（不误放行、不静默当删除）；彻底去上限需改流式 hash（`spawn` stdout 接 `crypto.createHash`），已记 TODO 注释，罕见 + 安全，后置 | 🟡 | 2026-08-20 |
| 目录评审逐文件分块（修「审项目卡死超时」根因） | `reviewDir()`：`--dir` 不再把所有文件拼成单一巨 payload 一次性发（曾致审整个项目卡死超时），改**逐文件串行**审（每文件内部仍按 800 行 `reviewFile` 分块 + 行号偏移），issue 强制写回相对路径 `file`，逐文件进度写 stderr（`[model] [i/N] relPath`，防"审目录=盲等"）；`review-runner.mjs` CLI `--dir` + `jobs.mjs runAudit` dir 分支都路由到 `reviewDir`；`review()` 的 dir 分支保留作向后兼容。摸底实测钉死根因：单文件 800 行 16s、目录 4 文件 1600 行 16s、目录 scripts 40 文件（46 万字符）卡死 >10min。`review-runner.test.mjs` 新增 7 用例（逐文件非拼球/串行不重叠/file 强制相对路径/空目录/最高 severity/单文件失败不拖垮/进度输出），全量 784 单测 + guard 绿 | 🟢 | 2026-08-21 |
| evaluate 文档漂移修复（D：precision 免费常驻 /evaluate + --arbitrate 纯裁决） | 根因：`evaluate.md` 说「--arbitrate 给 precision」，但 CLI 的 `--arbitrate` 实际走 `adjudicateLedger`（裁决账本），`evaluateModels({arbitrate:true})` 算 precision 的分支生产入口够不到（死代码）。修法：① 新增 `computePrecision(log)`（免费从账本 verdict+models 算 per-model precision + 独有真）；② `/evaluate` 非 arbitrate 输出合并成六列表（run/avg/共识/precision/独有真/样本）；③ 删 `evaluateModels` 的 `arbitrate` 分支（`dedupFindings`/`adjudicate`/`adjudicateLedger` 保留）；④ `--arbitrate` 保持纯裁决；⑤ `evaluate.md` 对齐 + `docs-consistency.test.mjs` 加防漂移断言。TDD：删 12 旧 arbitrate 用例 + 新增 `computePrecision` 6 用例 + cli 合并输出 1 用例；806 单测 + guard 绿。**复审假阳留痕**：`/verify` diff 复审报 high（kimi 3 high + qwen 1 medium），opencode 代码级终审 4 条全假阳——①「--arbitrate 变 no-op」假（`--arbitrate` 分支 evaluate-models.mjs:460-485 原样未动仍调 adjudicateLedgerFn，kimi 只审 hunks 没看到）；②「loadVerdicts 无文件崩溃」假（`loadVerdicts` 对 ENOENT 返回 `[]`，verdict-log.mjs:49）；③「evaluateModels 收窄 API 破坏性」假（`rg evaluateModels\b` 全仓零外部调用方）；④「cli 加 loadLedger 没处理异常」假（cli 整体 try/catch + loadVerdicts ENOENT 兜底）。据此判定无真 high/medium，`--mark --verdict clean` 解除 | 🟢 | 2026-08-21 |
