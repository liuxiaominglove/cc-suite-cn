---
description: 修复闭环 — 找 bug → 批判(qwen) → 裁决(hy3) → 终审修(opencode) → 验证 → 复审(门控)
argument-hint: <path>
agent: build
---
...
对 `$ARGUMENTS`（文件或目录路径，同 `/audit`）跑完整闭环：**找 → 批判 → 裁 → 修 → 验 → 复审**。
...
**Step 5 完成后，立即停止。** 提交前 5 步结果（修了哪些 bug + 测试证据 + 三节汇报，见 `AGENTS.md` 汇报惯例），并**显式提醒用户**：

> 「第 6 步【复审】还没做——复审 = 拿 Step 4 修完后的代码，**重新真实跑一遍 Step 1-5**（找 bug → 批判 → 裁决 → 终审修 → 验证），让多模型独立确认『修对了 + 无回归 + 无遗漏』。是否要做？」

**用户同意后**才进入 Step 6；用户说"先不 / 跳过"则记录「⏸️ 尚未复审」并收尾。

## Step 6: 复审（重跑 Step 1-5，无第 6 步）

把 **Step 4 修完后的代码当作新目标**，真实重跑 Step 1-5（找 bug → 批判 → 裁决 → 终审修 → 验证）。这次**没有 Step 6**（复审只跑一轮，不递归）。

- 复审里若再揪出真 bug → 按 Step 4 的 TDD 再修（新 commit，不 amend）。
- 复审干净 → 报告「🟢 已复审」，完整收尾。

## ★ 复审后：问 commit

复审结束后，向用户报告「🟢 已复审」+ 三节汇报，并**显式问「要 commit 吗？」**：
- 用户要 → `git add <明确列出的文件>`（禁 `git add .`）→ `git commit`，提交信息一句话说清改了什么
- 用户不要 → 收尾，改动留在工作区
- 用户之前跳过复审就要求 commit → 可以 commit，但必须显式提醒「⏸️ 尚未复审」，不掩盖
...
- **复审门控**：Step 5 后必须硬暂停 + 问用户；复审未做（或用户说跳过）必须显式标「⏸️ 尚未复审」，禁止用「已修复」「全流程完成」掩盖。
...
- 写后不自动合并；**复审后问 commit，用户同意才 commit**（跳过复审就要求 commit 时，须显式标「⏸️ 尚未复审」）
