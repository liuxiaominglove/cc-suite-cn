...
- **precision / 独有真**：来自裁决账本（`.cc-suite-cn/verdict-log.json` 里 hy3 已裁决的 finding），**免费只读**——裁决结果已落库，不重新花钱。
...
hy3 逐条裁决账本里**尚未裁决**的 finding（含 `verdict` 为空和 `uncertain` 的），并把 `verdict/evidence/codeHash` 落库。输出「已裁决 N 条（真 X / 假 Y / 不确定 Z）」。

> 这是 `/fix` 的硬门槛：只修 hy3 判 `true` 的 finding。裁决花钱（起 hy3）且耗时（一条一调）。

## Step 3: 汇报

- **run / avg/run**：跑了多少次审计、平均每次报几条（数量）
- **共识率**：报的 finding 里有多少也被别的施工队发现（质量，免费）
- **precision**：hy3 判定为真的比例（质量，从账本免费读）
